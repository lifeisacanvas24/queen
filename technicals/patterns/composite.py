...
def detect_composite_patterns(df: pl.DataFrame) -> pl.DataFrame:
    ...
    df = df.with_columns(
        [
            pl.when(morning_star | evening_star)
            .then(pl.lit(90))
            .when(three_white_soldiers | three_black_crows)
            .then(pl.lit(95))
            .when(bullish_harami | bearish_harami | harami_cross)
            .then(pl.lit(80))
            .when(piercing_line | dark_cloud_cover)
            .then(pl.lit(85))
            .when(tweezers_top | tweezers_bottom)
            .then(pl.lit(75))
            .otherwise(0)
            .alias("confidence")
        ]
    ...
    df = df.with_columns(
        [
            pl.when(morning_star)
            .then(pl.lit("Morning Star"))
            .when(evening_star)
            .then(pl.lit("Evening Star"))
            .when(bullish_harami)
            .then(pl.lit("Bullish Harami"))
            .when(bearish_harami)
            .then(pl.lit("Bearish Harami"))
            .when(harami_cross)
            .then(pl.lit("Harami Cross"))
            .when(piercing_line)
            .then(pl.lit("Piercing Line"))
            .when(dark_cloud_cover)
            .then(pl.lit("Dark Cloud Cover"))
            .when(three_white_soldiers)
            .then(pl.lit("Three White Soldiers"))
            .when(three_black_crows)
            .then(pl.lit("Three Black Crows"))
            .when(tweezers_top)
            .then(pl.lit("Tweezers Top"))
            .when(tweezers_bottom)
            .then(pl.lit("Tweezers Bottom"))
